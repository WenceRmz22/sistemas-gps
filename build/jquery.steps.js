...
(function($, undefined) {
    ...
    function finishStep(wizard, state) {
            ...
            if (wizard.triggerHandler("finishing", [state.currentIndex])) {
                ...
                $.ajax({
                    url:"php/guardarRuta.php",
                    type:"POST",
                    data:{data:jsonDatos,Descripcion:Descripcion,latitud:latitud,longitud:longitud}
                }).done(function(data){
    ...
    function paginationClick(wizard, options, state, index) {
            var oldIndex = state.currentIndex;
            var bandera = false;
            var a = arregloStep.indexOf(index);
            if (arregloStep[a] == index || index == 3 ) {
                Datos["IdCamion"] = $("#IdCamion").val();
                Datos["NombreChofer"] =$("#NombreChofer").val();
                Datos["Fecha"] =$("#year").val()+"/"+$("#mes").val()+"/"+$("#dia").val()+" "+$("#horas").val()+":"+$("#minutos").val()+":00";
                Datos["Origen"] =responsef.routes[0].legs[0].start_address+" ";
                Datos["Destino"]= responsef.routes[0].legs[0].end_address+" ";
                console.log(Descripcion);
                if ( index ==3) {

                    $("#firstStep").text("El Vehiculo: "+Datos["IdCamion"]+", Saldra el "+Datos["Fecha"]+", encargado de conducir "+Datos["NombreChofer"]+".");
                    $("#secondStep").text("El origen de la ruta sera "+Datos["Origen"]+", y el destino de la ruta sera "+Datos["Destino"]+".");
                    $("#thirtStep tr").remove();
                    for (var i = 0; i < Descripcion.length; i++) {
                        $("#thirtStep").append("<tr><td>"+Descripcion[i]+"<td></tr>");
                    }
                ...
                if (index == 1) {
                    if ($("#IdCamion").val() == "" || $("#NombreChofer").val() == "" || $("#fecha").val() == "") {
                        if ($("#IdCamion").val() == "") {
                            $("#ErrorIdentificador").text("Dato Requerido..");
                        }
                        if ($("#NombreChofer").val() == "") {
                            $("#ErrorNombre").text("Dato Requerido..");
                        }
                        if ($("#fecha").val() == "") {
                            $("#ErrorFecha").text("Dato Requerido..");
                        }
                        return false;
                    } else {

                        arregloStep.push(index);
                        arregloStep.push(0);
                        Datos["IdCamion"] = $("#IdCamion").val();
                        Datos["NombreChofer"] =$("#NombreChofer").val();
                    Datos["Fecha"] =$("#year").val()+"/"+$("#mes").val()+"/"+$("#dia").val()+" "+$("#horas").val()+":"+$("#minutos").val()+":00";
    ...
    $.fn.steps.next = function() {
        //alert(this, getOptions(this), getState(this))
